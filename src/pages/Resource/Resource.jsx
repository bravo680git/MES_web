...
const handler = {
    ...
    navigateLinks: [paths.resource + "/equipment", paths.resource + "/material"],
}
...
function Resource() {
    ...
    useEffect(() => {
        callApi([resourceApi.equipment.getEquipments(), resourceApi.material.getMaterials()], (res) => setResData(res))
    }, [callApi])

    return (
        <div data-component="Resource" className="container flex h-full flex-wrap">
            {resData.map((data, index) => (
                <div className="h-full w-1/2 pr-5 last:pr-0 xl:mb-4 xl:h-full xl:w-full xl:pr-0" key={index}>
                    <ResourceItem
